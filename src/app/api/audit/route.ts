...
export async function POST(request: NextRequest) {
  ...
  if (usageData.effectivePlan === "free") {
    return NextResponse.json(
      { error: "Listing audits are available on paid plans.", code: "FEATURE_GATED" },
      { status: 402 }
    );
  }
  ...
  if (limit !== null && used >= limit) {
    return NextResponse.json(
      { error: `You've used all ${limit} audits for this month. Upgrade your plan to continue.`, code: "LIMIT_EXCEEDED" },
      { status: 402 }
    );
  }
